...
help-git: ## Show Git operation targets
	...
	@printf "  make git-diff-dev         Compare dev against rc when both branches exist\n"
	@printf "  make git-diff-rc          Compare rc against master when both branches exist\n"
	...
	@printf "  make git-configure-release-labels\n"
	@printf "                            Synchronize the Release Please lifecycle labels on GitHub\n"
...
git-protect-default-branch: ## Require PRs and successful CI before updating the GitHub default branch
	...
	printf "  $(DIM)Administrators cannot bypass the rule; conversations, force-pushes, and deletion are blocked.\n$(NC)"
...
git-configure-release-labels: ## Create or update the Release Please lifecycle labels in GitHub
	...
	if [ "$(DRY_RUN)" = "1" ]; then \
		printf "  ▶ [dry-run] would synchronize Release Please lifecycle labels\n"; \
		exit 0; \
	fi; \
	gh label create 'autorelease: pending' --color '1D76DB' --description 'Release Please has an open release pull request' --force; \
	gh label create 'autorelease: tagged' --color '0E8A16' --description 'Release Please published this release' --force; \
	printf "$(GREEN)  ✓ Release Please lifecycle labels synchronized$(NC)\n"
...
git-setup: ## Clone a repo as bare + create all worktrees with upstream (use REPO=url)
	...
	@if [ -z "$(REPO)" ]; then \
		...
		printf "  usage:  $(BLUE)make git-setup REPO=git@github.com:robert-flo/dotfiles.git$(NC)\n\n"; \
